...
export const Templates = {
  ...
  videoCard: (video, index, formattedDuration, resolutionText, isAuth, cleanDisplayUrl) => {
    ...
    <div class="video-card animate-fade-in" style="position: relative;">
      ...
      <button class="btn select-video-btn" style="margin-top:10px; padding:8px;" data-index="${index}">
    ...
    return `
      ...
      <div class="animate-fade-in">
        <div class="tech-input-group">
          <input type="text" id="inp-search" class="tech-input" value="${
            state.searchQuery
          }" placeholder="Search database..." autocomplete="off">
          <button id="btn-go-search" class="icon-btn" style="border-radius:0; border-left:1px solid var(--border-dim); width:32px;">
            ${ICONS.SEARCH}
          </button>
        </div>
        ...
        ${
          ...
          <div style="display:flex; gap:10px; margin-bottom:12px;">
            <div style="flex:1;">
              <label class="text-xs" style="color:var(--text-dim); display:block; margin-bottom:4px;">SEASON</label>
              <div class="tech-input-group" style="margin-bottom:0;">
                <input type="number" id="inp-season" class="tech-input" value="${state.season}" min="1" style="text-align:center;">
              </div>
            </div>
            <div style="flex:1;">
              <label class="text-xs" style="color:var(--text-dim); display:block; margin-bottom:4px;">EPISODE</label>
              <div class="tech-input-group" style="margin-bottom:0;">
                <input type="number" id="inp-episode" class="tech-input" value="${state.episode}" min="1" style="text-align:center;">
              </div>
            </div>
          </div>`
            : ""
      ...
      <div class="search-res-layout">
        ...
        <div class="res-info">
          ...
          <div style="display:flex; align-items:center; gap:6px; margin-bottom:4px;">
            ...
            ${
              r.is_local
                ? `<span class="badge badge-local">LIBRARY</span>`
  ...
  taskItem: (task) => {
    ...
    <div class="task-card animate-fade-in">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
        <div style="font-size:11px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:180px;">
          ${task.originalName}
        </div>
        <span class="badge" style="background:transparent; border:1px solid ${statusColor}; color:${statusColor};">
      ...
      ${
        task.error
          ? `<div style="font-size:10px; color:var(--status-error); margin-top:4px; font-family:var(--font-mono);">// ${task.error}</div>`
          : ""
      }
    </div>
  `;
  ...
  successScreen: () => `
    <div class="animate-fade-in" style="flex:1; display:flex; flex-direction:column; justify-content:center; align-items:center;">
      <div style="width:60px; height:60px; border-radius:50%; background:rgba(16, 185, 129, 0.1); color:var(--status-success); display:flex; align-items:center; justify-content:center; margin-bottom:20px; box-shadow:0 0 15px rgba(16, 185, 129, 0.2);">
        ${ICONS.SUCCESS}
      </div>
      <h2 style="font-size:16px; margin-bottom:8px; color:white;">Sequence Initiated</h2>
      <p style="font-size:12px; color:var(--text-muted); text-align:center; margin-bottom:30px;">
        The ingest protocol has been successfully transmitted to the server.
      </p>
